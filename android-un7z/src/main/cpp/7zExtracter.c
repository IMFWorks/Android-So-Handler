...
extractStream(JNIEnv *env, ISeekInStream *seekStream, const char *destDir,
    ...
    if (callback != NULL) {
        ...
        onProgress = (*env)->GetMethodID(env, callbackClass, "onProgress", "(Ljava/lang/String;Ljava/lang/String;J)V");
    }
    ...
    if (res == SZ_OK) {
        ...
        */
        ...
        for (i = 0; i < db.NumFiles; i++) {
            ...
            CallJavaStringStringLongMethod(env, callback, onProgress,
                                     destDir, (char *) fileNameBuf.data, fileSize);
